        ...
        for listener, sub_group in group.groupby('listener'):
            for src_node_id, src_group in sub_group.groupby('SrcNodeId'):
                ...
                result.append({
                    ...
                    'IsSink': is_sink,
                    'PacketLossRate': (rec_ack_count[src_node_id] / (rec_udp_count[src_node_id] + arp_reply_count)) if rec_udp_count[src_node_id] != 0 else 1,
                    'DataReportRate': (udp_count / 2000),
                    'DataForwardRate': ((fw_udp_count[src_node_id] - udp_count) / (2 * rec_udp_count[src_node_id])) if rec_udp_count[src_node_id] != 0 else 1,
                    'AvgSNR': avg_snr,
